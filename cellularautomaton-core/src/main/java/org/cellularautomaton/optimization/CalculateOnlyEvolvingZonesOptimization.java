...
public abstract class CalculateOnlyEvolvingZonesOptimization<StateType> extends
		AbstractOptimization<StateType> implements
		PreCalculationOptimization<StateType>,
		PreApplyingOptimization<StateType>,
		CellsSelectionOptimization<StateType> {
	...
	public Collection<ICell<StateType>> getCellsToManage() {
		if (cellsToCalculate == null) {
			cellsToCalculate = getAutomaton().getSpace().getAllCells();
		}
		
		if (getAutomaton().isReadyForCalculation()) {
			return cellsToCalculate;
		} else if (getAutomaton().isReadyForApplying()) {
